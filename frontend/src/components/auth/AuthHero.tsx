  ...
  return (
    <section className="px-1 py-4 lg:px-2 lg:py-6">
      <div className="flex flex-col gap-6">
        <div className="rounded-[28px] border border-panel-foreground/20 bg-gradient-to-br from-panel to-panel/80 p-8 text-panel-foreground shadow-2xl sm:p-10">
          <div className="flex flex-col gap-5">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-panel-foreground/70">
              Phase 3 Frontend
            </p>
            <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">{title}</h1>
            <p className="max-w-2xl text-base leading-7 text-panel-foreground/80">{subtitle}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {stats.map((stat) => (
            <Card className="rounded-[24px] bg-card shadow-sm" key={stat.label}>
